...
def create_publisher(x, y,pub_ip):
	context = zmq.Context()
	socket= context.socket(zmq.PUB)
	socket.bind("tcp://"+pub_ip+":%s" %x)
	time.sleep(40)
	t=0
...
def publisher_connect(pub_num, topic,owner, number,pub_ip):
	poller=zmq.Poller()
	x=0
	context = zmq.Context()
	socket_sub= context.socket(zmq.SUB)
	socket_sub.connect("tcp://"+pub_ip+":%s" %pub_num)
			...
			if(x<number) or (number==0):
				...
				if(message=='dieing'):
					if(pub_num==owner[0]):
						...
						socket_sub1.connect("tcp://"+pub_ip+":%s" %owner[1])
						...
						while True:
							...
							if socket_sub1 in event:
								...
								if(message2=='dieing'):
									...
									socket_sub2.connect("tcp://"+pub_ip+":%s" %owner[2])
						...
						socket_sub3.connect("tcp://"+pub_ip+":%s" %owner[0])
						...
						while True:
							...
							if socket_sub3 in event:
								...
								if(message4=='dieing'):
									...
									socket_sub4.connect("tcp://"+pub_ip+":%s" %owner[2])
						...
						socket_sub.connect("tcp://"+pub_ip+":%s" %owner[0])
						...
						while True:
							...
							if socket_sub in event:
								...
								if(message6=='dieing'):
									...
									socket_sub6.connect("tcp://"+pub_ip+":%s" %owner[2])
